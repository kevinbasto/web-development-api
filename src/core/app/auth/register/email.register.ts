...
export class EmailRegister {
    ...
    async register( registerData : EmailRegisterDto, lang : string ) : Promise<SystemMessageDto> {
        ...
        let token = this.createAccountAndUser(registerData, lang).catch(error => { throw error });

        return;
    }

    private async validateAccount( email : string, lang : string ){
        let account = this.accountsRepo.getAccountByEmail(email).catch(async() => { throw await this.getDatabaseErrorMessage(lang) });
        await this.validateAccountExists(account, lang).catch((error : Exception) => { throw error.getException()});
    }

    private async getDatabaseErrorMessage(lang : string){
        let name : string = await this.translater.getTranslation(lang, `register.${EmailRegisterMessage.DATABASE_EXCEPTION_NAME}`);
        let message : string = await this.translater.getTranslation(lang, `register.${EmailRegisterMessage.DATABASE_EXCEPTION_MESSAGE}`);
        return new DatabaseException(name, message).getException();
    }

    private async validateAccountExists(account : any, lang : string){
        let name : string = await this.translater.getTranslation(lang, `register.${EmailRegisterMessage.ACCOUNT_DUPLICATED_NAME}`);
        let message : string = await this.translater.getTranslation(lang, `register.${EmailRegisterMessage.ACCOUNT_DUPLICATED_MESSAGE}`);
        if(account)
            throw new UserDuplicatedException( name, message );
    }
